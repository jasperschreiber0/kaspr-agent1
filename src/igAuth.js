 ...
 * client_id is their row UUID in the clients table — passed as state so we
 * know which client to update when the callback comes back.
 */
router.get('/connect', (req, res) => {
  ...
  url.searchParams.set('state', clientId); // carries client UUID through OAuth round-trip
 ...
 * 3. Fetch their IG account ID
 * 4. Save both to clients table
 */
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    console.log(`[ig-auth] Client ${clientId} connected IG account ${igAccountId}`);
